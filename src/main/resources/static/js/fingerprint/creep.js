...
`;
    ...
    if (!IS_WORKER_SCOPE) {
        ...
        console.log(`${propsSearched.length} API properties analyzed in ${PROTO_BENCHMARK}ms (${lieList.length} corrupted)`);
    }
    ...
    */
    ...
    function getRawFingerprint(fp) {
        try {
            ...
            const analysisFP = {
                ...
                webglParams: (canvasWebgl?.parameters ?
                    '' + [
    ...
    async function getStatus() {
        const [batteryInfo, quota, scriptSize, stackSize, timingRes, clientLitter,] = await Promise.all([
            getBattery(),
            getStorage(),
            getScriptSize(),
            getMaxCallStackSize(),
    ...
    !async function () {
        ...
        const creep = {
            ...
            canvasWebgl: !fp.canvasWebgl || fp.canvasWebgl.lied ? undefined : (braveFingerprintingBlocking ? {
                parameters: {
